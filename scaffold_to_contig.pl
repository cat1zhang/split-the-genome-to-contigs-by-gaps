...
Usage: perl split_scaffold.pl [options] <input_scaffold_file.fa>
        ...
        -out <str>  three alternative types: contig_seq, contig_coor default=contig_seq
...
Example: perl scaffold_to_contig.pl scaffold.fa > contig.fa
...
while (<IN>) {
        ...
        for (my $i=0; $i<$frag_num; $i++) {
                ...
                if ($frag[$i][3]>=$cutoff) {
                        print ">$contig_id  $chr $seq_len  $frag[$i][1] $frag[$i][2] $frag[$i][3]\n$frag[$i][0]" if ($out_type eq "contig_seq");
                        print "$contig_id  $chr $seq_len  $frag[$i][1] $frag[$i][2] $frag[$i][3]\n" if ($out_type eq "contig_coor");
                        $mark++;
                }
